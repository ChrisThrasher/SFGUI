...
std::unique_ptr<RenderQueue> BREW::CreateEntryDrawable( std::shared_ptr<const Entry> entry ) const {
	...
	queue->Add(
		Renderer::Get().CreatePane(
			sf::Vector2f( 0.f, 0.f ),
			sf::Vector2f( entry->GetAllocation().size.x, entry->GetAllocation().size.y ),
			border_width,
			background_color,
			border_color,
			-border_color_shift
	...
	vis_label.setPosition( {text_padding, entry->GetAllocation().size.y / 2.f - line_height / 2.f} );
	...
	if( entry->HasFocus() && entry->IsCursorVisible() ) {
		...
		queue->Add(
			Renderer::Get().CreateRect(
				sf::FloatRect(
					{metrics.x + text_padding, entry->GetAllocation().size.y / 2.f - line_height / 2.f},
