...
std::unique_ptr<RenderQueue> BREW::CreateCheckButtonDrawable( std::shared_ptr<const CheckButton> check ) const {
	...
	queue->Add(
		Renderer::Get().CreatePane(
			sf::Vector2f( 0.f, check->GetAllocation().size.y / 2.f - box_size / 2.f ),
	...
	if( check->IsActive() ) {
		...
		queue->Add(
			Renderer::Get().CreateRect(
				sf::FloatRect(
					{box_size / 2 - check_size / 2, check->GetAllocation().size.y / 2.f - box_size / 2.f + diff / 2.f},
	...
	if( check->GetLabel().getSize() > 0 ) {
		...
		text.setPosition(
			{box_size + spacing,
			 check->GetAllocation().size.y / 2.f - metrics.y / 2.f}
		);
